...
enum Commands {
    ...
    MergeLog {
        ...
        /// Allow merge even when com.contrail.daemon appears to be running.
        #[arg(long)]
        allow_daemon_running: bool,
    },
}
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::MergeLog {
            file,
            allow_daemon_running,
        }) => run_merge(file, allow_daemon_running),
    }
}
...
fn run_merge(file: PathBuf, allow_daemon_running: bool) -> Result<()> {
    let config = ContrailConfig::from_env()?;

    if !allow_daemon_running && is_contrail_daemon_running() {
        anyhow::bail!(
            "com.contrail.daemon is running; stop it before merge (or pass --allow-daemon-running)"
        );
    }
...
mod tests {
    ...
    #[test]
    fn merge_log_defaults_to_guarding_against_running_daemon() {
        let parsed = Cli::try_parse_from(["importer", "merge-log", "/tmp/input.jsonl"]).unwrap();
        let Some(Commands::MergeLog {
            allow_daemon_running,
            ..
        }) = parsed.command
        else {
        ...
        assert!(!allow_daemon_running);
    }

    #[test]
    fn merge_log_allows_override_flag() {
        let parsed = Cli::try_parse_from([
            "importer",
            "merge-log",
            "/tmp/input.jsonl",
            "--allow-daemon-running",
        ])
        .unwrap();
        let Some(Commands::MergeLog {
            allow_daemon_running,
